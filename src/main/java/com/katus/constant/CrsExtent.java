package com.katus.constant;

/**
 * @author Sun Katus
 * @version 1.0, 2020-11-13
 */
public final class CrsExtent {
    public static final Double[] EXTENT_3857 = new Double[]{-20026376.39, 20026376.39, -20048966.10, 20048966.10};
    public static final Double[] EXTENT_4326 = new Double[]{-180.0, 180.0, -90.0, 90.0};
    public static final Double[] EXTENT_4490 = new Double[]{73.62, 134.77, 16.7, 53.56};
    public static final Double[] EXTENT_4528 = new Double[]{40347872.25, 40599933.05, 2703739.74, 5912395.20};
}
